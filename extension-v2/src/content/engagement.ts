/**
 * Engagement tracker — measures how much attention a page/chat actually got, so
 * a skimmed tab and a deeply-read one aren't weighted the same. Ported from the
 * v1 extension.
 *
 *   dwellTimeMs     — active time on the page (paused when the tab is hidden)
 *   scrollDepth     — furthest fraction of the page scrolled (0–1)
 *   readingDepth    — fraction of content actually read, weighted by pace
 *   engagementLevel — the attention stage: latent → passive → active → deep
 *   baseWeight      — capture weight derived from the stage (0.2 / 1.0 / 2.0)
 */
...
export class EngagementTracker {
  ...
  /** Content grew (e.g. a chat gained messages) — keep reading-depth honest. */
  ...
  // reading_depth: fraction of content actually read, weighted by pace. Can
  // exceed 1.0 (up to 1.5) when the user re-reads or reads very carefully.
  ...
  // base_weight upgrade allowed by the measured engagement.
